...
$(BUILD_DIR)/similarity-subsumers.ofn: $(BUILD_DIR)/anatomical_entities.txt
	mkdir -p $(dir $@) && dosdp-tools generate --generate-defined-class=true --obo-prefixes=true --template=patterns/semantic_similarity_subsumers.yaml --infile=$< --outfile=$@
